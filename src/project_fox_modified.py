...
for prop_name, results in all_results.items():
    ...
    for k in K_VALUES:
        ...
        for strategy in strategy_order:
            ...
            influence_row[f"{strategy}_Total_Influence"] = r["total_activated"]
            influence_row[f"{strategy}_Positive_Influence"] = r["positive_inf"]
            influence_row[f"{strategy}_Net_Influence"] = r["net_influence"]

            quality_row[f"{strategy}_Strong_Positive"] = r["strong_positive"]
            quality_row[f"{strategy}_Strong_Negative"] = r["strong_negative"]
            quality_row[f"{strategy}_WNS"] = r["WNS"]
            quality_row[f"{strategy}_CICR"] = r["CICR"]

        influence_rows.append(influence_row)
    ...
    with open(fpath, "w") as f:
        f.write(f"=== {prop_name} - Table 1: Total Influence / Positive Influence / Net Influence ===")
        influence_table.to_csv(f, index=False)

        f.write("")
        f.write(f"=== {prop_name} - Table 2: Strong Positive / Strong Negative / WNS / CICR ===")
        quality_table.to_csv(f, index=False)
