...
TABLE1_COLS = ["selection_strategy", "k", "total_activated", "positive_inf", "net_influence"]
TABLE2_COLS = ["selection_strategy", "k", "strong_positive", "strong_negative", "WNS", "CICR"]
...
for prop_name, results in all_results.items():
    ...
    table1 = results_df[TABLE1_COLS].sort_values(["selection_strategy", "k"])
    table2 = results_df[TABLE2_COLS].sort_values(["selection_strategy", "k"])

    with open(fpath, "w") as f:
        f.write(f"=== {prop_name} — Table 1: Activation Metrics ===\n")
        table1.to_csv(f, index=False)
        f.write(f"\n=== {prop_name} — Table 2: Quality Metrics ===\n")
        table2.to_csv(f, index=False)

    print(f"  {fname} saved  ({len(results_df)} rows)")
